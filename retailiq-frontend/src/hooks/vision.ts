...
export type OcrTerminalState = 'FAILED' | 'COMPLETED';

export const isTerminalState = (status?: string | null) => Boolean(status && ['FAILED', 'COMPLETED'].includes(status));
...
export const useOcrJobQuery = (jobId: string | null) => useQuery({
  ...
  refetchInterval: (query: { state: { data: GetOcrJobResponse | undefined } }) => {
    ...
    return isTerminalState(status) ? false : 3000;
  },
  staleTime: 0,
});
