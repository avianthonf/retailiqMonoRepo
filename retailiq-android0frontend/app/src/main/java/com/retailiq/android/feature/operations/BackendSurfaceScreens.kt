...
fun StoreAdminScreen(repository: RetailIqRepository) {
    val state = rememberBackendResource(repository) { storeSnapshot() }
    ...
    AppScreen(
        ...
        when {
            ...
            state.error != null -> Text(text = state.error, color = MaterialTheme.colorScheme.error)
                ...
                val data = state.data ?: return@AppScreen
                ...
                Card(shape = RoundedCornerShape(24.dp)) {
                    Column(modifier = Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        data.categories.forEach { category ->
                            RecordRow(
                                category.name,
                                if (category.active) "Active category" else "Archived category",
                                "${category.gstRate}%",
                            )
                        }
                    }
...
fun CustomerCenterScreen(repository: RetailIqRepository) {
    val state = rememberBackendResource(repository) { customerSnapshot() }
...
fun SupplierCenterScreen(repository: RetailIqRepository) {
    val state = rememberBackendResource(repository) { supplierSnapshot() }
...
fun ForecastingSurfaceScreen(repository: RetailIqRepository) {
    val state = rememberBackendResource(repository) { forecastingSnapshot() }
        ...
        when {
            ...
            state.error != null -> Text(text = state.error, color = MaterialTheme.colorScheme.error)
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.fillMaxWidth()) {
                    StatCard(
                        "Reorder",
                        if (data.suggestion.shouldReorder) "Yes" else "No",
                        "Suggested qty ${data.suggestion.suggestedOrderQty}",
                        Modifier.weight(1f),
                    )
                    StatCard(
                        "Demand",
                        data.suggestion.forecastedDemand.toString(),
                        "Lead time ${data.suggestion.leadTimeDays} days",
                        Modifier.weight(1f),
                    )
                }
...
fun ReceiptsSurfaceScreen(repository: RetailIqRepository) {
    val state = rememberBackendResource(repository) { receiptsSnapshot() }
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.fillMaxWidth()) {
                    Button(
                        onClick = {
                            scope.launch {
                                actionError = null
                                latestJob = runCatching { repository.createReceiptPrintJob() }
                                    .onFailure { actionError = it.message ?: "Print job creation failed." }
                                    .getOrNull()
                            }
                        },
...
fun DeveloperConsoleScreen(repository: RetailIqRepository) {
    val state = rememberBackendResource(repository) { developerSnapshot() }
...
fun SystemStatusScreen(repository: RetailIqRepository) {
    val state = rememberBackendResource(repository) { systemStatus() }
