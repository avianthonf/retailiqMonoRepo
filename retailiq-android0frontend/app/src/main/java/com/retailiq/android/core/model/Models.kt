...
object RetailIqModuleCatalog {
    ...
    fun defaultModules(): List<ModuleSpec> = listOf(
        ...
        ModuleSpec(
            ...
            backendPrefix = "/api/v1/staff-performance",
            status = ModuleStatus.Ready,
            category = ModuleCategory.Operations,
        ...
        ModuleSpec(
            ...
            backendPrefix = "/api/v1/developer",
            status = ModuleStatus.Ready,
            category = ModuleCategory.Platform,
        ...
        ModuleSpec(
            ...
            backendPrefix = "/api/v1/health",
            status = ModuleStatus.Ready,
            category = ModuleCategory.Platform,
            heroMetric = "3 services healthy",
            description = "Surface health and team ping status directly in the mobile shell for quick diagnostics.",
            records = listOf(
                ModuleRecord("API", "Primary app endpoints", "ok"),
                ModuleRecord("Database", "Persistent data layer", "ok"),
                ModuleRecord("Redis", "Cache and queue layer", "ok"),
            ),
            actions = listOf(
                ModuleAction("Open health", "Confirm API, database, and cache health."),
